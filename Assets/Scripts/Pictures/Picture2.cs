...
public class Picture2 : MonoBehaviour
{
    ...
    private void OnMouseUp()
    {
        ...
        if (Mathf.Abs(transform.localPosition.x - correctForm2.transform.localPosition.x) <= 0.5f &&
            Mathf.Abs(transform.localPosition.y - correctForm2.transform.localPosition.y) <= 0.5f)
        {
            transform.position = new Vector3(correctForm2.transform.position.x,
                                                correctForm2.transform.position.y,
                                                correctForm2.transform.position.z);
            finished = true;
        }
        else
